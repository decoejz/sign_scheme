...
int verify_rsa(uint8_t *msg_raw, uint8_t *msg_signed, int total_len, pki_t public_key)
{
    ...
    int msg_len = total_len - RSA_SIGN_HEADER_SIZE - sigma_len;
    unsigned char sigma[sigma_len];

    // (siglen (int) + msg + sigma)
    memcpy(msg_raw, msg_signed + RSA_SIGN_HEADER_SIZE, msg_len);
    memcpy(sigma, msg_signed + RSA_SIGN_HEADER_SIZE + msg_len, sigma_len);
